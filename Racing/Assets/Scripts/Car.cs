...
public class Car : MonoBehaviour
{
    ...
    [field: SerializeField] public float topSpeed { private set; get; }
    ...
    private void Update()
    {
        float verticalInputAxis = Input.GetKey(KeyCode.B) ? 0 : Input.GetAxisRaw("Vertical");
        kmph = transform.InverseTransformDirection(rb.velocity).z * 3.6f;
        float potentialEngineForce = Mathf.InverseLerp(0, topSpeed, topSpeed - kmph) * acceleration;

        engineForceOutput = verticalInputAxis * potentialEngineForce;

        //audio
        engineAudio.pitch = Mathf.Lerp(engineAudio.pitch, 2.5f * Mathf.InverseLerp(0, acceleration, (acceleration - potentialEngineForce) * Mathf.Abs(verticalInputAxis)) + .5f, Time.deltaTime * 3);
